...
var _default = (0, _assign.default)({
  ...
  isEmails: function isEmails(email) {
  ...
  isPhoneFixation: function isPhoneFixation(PhoneFixation) {
    return /([0-9]{3,4}-)?[0-9]{7,8}/.test(PhoneFixation);
  },
  IdCodeValid: function IdCodeValid(code) {
    //身份证号合法性验证
    //支持15位和18位身份证号
    //支持地址编码、出生日期、校验位验证
    var city = {
  ...
  isIdCard: function isIdCard(code) {
    return this.IdCodeValid(code).pass;
  },
  parseQuery: function parseQuery() {
    var query = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : location.href;
    var bb = query.split("?");
  ...
  isIllegality: function isIllegality(PhoneFixation) {
    return /^[a-zA-Z0-9]+$/i.test(PhoneFixation);
  },
  templateParsing: function templateParsing(d, key, val, options) {
    key = key || '';
    val = val || '';
    options = options || {};
    options.t1 = '{{';
  ...
  exactNum: function exactNum(value) {
    if (value === undefined) {
      return;
    }
  ...
  //数组去重
  dereplicationToArray: function dereplicationToArray(data) {
    var res = [];
    var json = {};
  ...
  //过滤重复数据，并保留重复数据
  filterDs: function filterDs(data) {
    var retlist = [];
  ...
  //图片上传兼容【苹果拍照图片旋转】
  imagesRotate: function imagesRotate(f, fn) {
    ...
    file.onload = function (e) {
      try {
        ...
        while (offset < len) {
          if (view.getUint16(offset, false) == 0xFFE1) break;else offset += 2;
        }
  ...
  //不四舍五入
  NoRound: function NoRound(a, length) {
    length = length || 0;
    var b = '';
    var index = a.indexOf(".");
    var index2 = 0;
   ...
   * 判断是否手ios或android
   * */
  isIosAndroid: function isIosAndroid() {
    var bool = true;
   ...
   * */
  SearchParentKey: function SearchParentKey(parent, keyName) {
    try {
      if (parent && keyName) {
